 ...
 */
export const selectBotCards = (hand, tikiLine, rules, difficulty) => {
  const line = [...tikiLine];

  // Sort rules to prioritize the one with highest points
  const sortedRules = [...rules].sort((a, b) => b.points - a.points);
  const primaryRule = sortedRules[0];
  const myColor = primaryRule.color;
  const myTargetPos = primaryRule.position - 1; // 0-indexed approximation for heuristics
  ...
  if (difficulty === "easy") {
    ...
    const easySelections = randoms.map((card) => fillTargetsRandomly(card, line));
    return easySelections.map((s) => ({
      ...s.card,
      target1: s.target1,
      target2: s.target2,
    }));
  ...
  if (selections.length < 2) {
    ...
    selections.push(fillTargetsStrategic(nextBest, line, rules));
  }
...
const fillTargetsStrategic = (card, line, rules) => {
  const myColor = rules[0].color;
  if (card.type === "move") {
